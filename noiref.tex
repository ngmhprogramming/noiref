...
Sparse Table   & O(N log N)     & X        & O(1)     & O(N log N) & Idempotent Functions (max, gcd) \\ \hline
...
\subsubsection{1D}
$O(N \log N)$ precomputation, $O(1)$ query.
\lstinputlisting[language=C++]{src/ds_sp_1d.cpp}

\subsubsection{2D}
$O(NM \log N \log M)$ precomputation, $O(1)$ query.
\lstinputlisting[language=C++]{src/ds_sp_2d.cpp}
...
Hungarian                 & O($V^{3}$)                                  & Weighted Bipartite Matching          \\ \hline
Edmond's                  & O($V^{3}$)                                  & General Matching                     \\ \hline
Weighted Edmond's         & O($V^{3}$)                                  & Weighted General Matching            \\ \hline
...
Max Flow / Min Cut        & O($V^2E$)                                   & Maximum Flow / Minimum Cut           \\ \hline
...
Unweighted MCBM, runs in $O(V^3)$.
\lstinputlisting[language=C++]{src/graph_mcbm_kuhn.cpp}

\subsubsection{Hopcroft-Karp}
Unweighted MCBM, runs in $O(E \cdot \sqrt{V})$.
\lstinputlisting[language=C++]{src/graph_mcbm_hopcroftkarp.cpp}

\subsubsection{Hungarian}
Min-weighted MCBM, runs in $O(V^3)$.
\lstinputlisting[language=C++]{src/graph_mcbm_hungarian.cpp}

\subsubsection{Edmond's}
Unweighted MCBM for general graph, runs in $O(V^3)$.
\lstinputlisting[language=C++]{src/graph_mcbm_edmonds.cpp}

\subsubsection{Weighted Edmond's}
Weighted MCBM for general graph, runs in $O(V^3)$.
\lstinputlisting[language=C++]{src/graph_mcbm_edmonds_weighted.cpp}
...
Using Dinic's Algorithm, runs in $O(V^2 E)$ for the general case.
For graph with unit capacity, $O(E\sqrt{E})$.
For unit network (unit capacity and either unique incoming or outgoing edge), $O(E\sqrt{V})$.
\lstinputlisting[language=C++]{src/graph_maxflow.cpp}
...
Knapsack - 0-K Binary    & O($S\sum\log K$)                &               \\ \hline
Knapsack - 0-K Queue     & O(NS)                           &               \\ \hline
Digit DP                 & O(D)                            &               \\ \hline
...
\subsubsection{0-K Binary Grouping}
Runs in $O(S \cdot \sum \log K)$.
\lstinputlisting[language=C++]{src/dp_knapsack_0kbinary.cpp}

\subsubsection{0-K Monotonic Queue}
Runs in $O(N \cdot S)$.
\lstinputlisting[language=C++]{src/dp_knapsack_0kqueue.cpp}
...
\subsubsection{$N^2$ DP}
\lstinputlisting[language=C++]{src/dp_lcs_n2.cpp}

\subsubsection{LIS Reduction}
Array $a$ cannot have duplicates.
\lstinputlisting[language=C++]{src/dp_lcs_lis.cpp}
...
Using Monotone Chain Algorithm, runs in $O(N \log N)$ time.
\lstinputlisting[language=C++]{src/math_convexhull.cpp}

\subsection{Furthest Pair of Points}
Using Rotating Calipers, runs in $O(H)$ time.
\lstinputlisting[language=C++]{src/math_furthestpoints.cpp}

\subsection{Nearest Pair of Points}
Using Shamos-Hoey, runs in $O(N \log N)$ time.
\lstinputlisting[language=C++]{src/math_nearestpoints.cpp}
...
Runs in $O(N)$.
\lstinputlisting[language=C++]{src/strings_manacher.cpp}

\subsection{Knuth-Morris-Pratt}
Runs in $O(N)$.
\lstinputlisting[language=C++]{src/strings_kmp.cpp}

\subsection{Z-Function}
Runs in $O(N)$.
\lstinputlisting[language=C++]{src/strings_zfunction.cpp}

\subsection{Rabin-Karp}
Runs in $O(N)$.
\lstinputlisting[language=C++]{src/strings_rabinkarp.cpp}

\subsection{Suffix Array}
Runs in $O(N \log N)$.
\lstinputlisting[language=C++]{src/strings_suffix_array.cpp}

\subsection{Suffix Tree}
Runs in $O(N \Sigma)$.
\lstinputlisting[language=C++]{src/strings_suffix_tree.cpp}
